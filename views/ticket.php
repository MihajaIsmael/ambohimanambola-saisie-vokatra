...
$labelsToPrint   = $ticketData['labelsToPrint'] ?? [];

// 🚀 DATA PREPARATION: Compile unique product metadata with target counts before destroying session payload
$synchronizedItems = [];
if (!empty($labelsToPrint)) {
    foreach ($labelsToPrint as $label) {
        $code = $label['product_code'] ?? '';
        if (!isset($synchronizedItems[$code])) {
            $synchronizedItems[$code] = [
                'product_code' => $code,
                'name'         => $label['name'] ?? 'Unknown',
                'price'        => (float)($label['price'] ?? 0),
                'qty'          => 1
            ];
        } else {
            $synchronizedItems[$code]['qty']++;
        }
    }
}
...
<body>
    ...
    <script>
        window.onload = function() {
            ...
            // 🚀 INTERPROCESS PIPELINE: Initialize secure cross-window browser broadcasting API
            try {
                const harvestChannel = new BroadcastChannel('harvest_printing_channel');
                const productsPayload = <?php echo json_encode(array_values($synchronizedItems)); ?>;
                
                // Transmit data array over channel context layer
                harvestChannel.postMessage({ products: productsPayload });
                harvestChannel.close();
            } catch (broadcastError) {
                console.error("Pipeline signaling failed:", broadcastError);
            }
